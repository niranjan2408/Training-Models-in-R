...
amazon= readLines(file.choose())

amazon <- stemDocument(amazon)

amazon_corpus <- Corpus(VectorSource(amazon))

inspect(amazon_corpus[c(10:15)])


corpus_clean = tm_map(amazon_corpus, tolower)
corpus_clean = tm_map(corpus_clean, removeNumbers)
corpus_clean = tm_map(corpus_clean, removeWords,c(stopwords("english"),stopwdrds))
corpus_clean = tm_map(corpus_clean, removePunctuation)
corpus_clean = tm_map(corpus_clean, stripWhitespace)
inspect(corpus_clean[c(10:15)])
...
####### unable to create bi gram plots (getting Warning message:ngrams argument is not used.) ################


# Bi gram word clouds

# Bi gram document term frequency 
dtm0_2 <- dfm(unlist(corpus_clean),ngrams = 2,verbose = F)
